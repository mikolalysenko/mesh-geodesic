...
var distance = require('../lib/geodesic.js');
...
vows.describe("distance.js").addBatch({
  ...
  '5x5 grid': {
    topic: gridMesh(5, 5, [0,0,0], [1,0,0], [0,1,0]),
    ...
    'testing all pairwise grid distances':  function(mesh) {
      ...
      for(var j=0; j<mesh.positions.length; ++j) {
        ...
        for(var i=0; i<mesh.positions.length; ++i) {
          ...
          assert.ok(Math.abs(distances[i] - d) < 1e-7, "Incorrect distance from " + j + "->" + i + ". Expected: " + d + ", got: " + distances[i]);
        }
      }      
    },
    
    'sphere': {
      topic: sphereMesh(10, 1.0),
